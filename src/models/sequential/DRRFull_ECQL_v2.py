...
class SlidingWindow:
    ...
    def update(self, user_id, item_emb, recommended_items=None, use_windowslen=1):
...
class DRRFull_ECQL_v2(GeneralModel):
    ...
    def parse_model_args(parser):
        ...
        parser.add_argument('--use_evidence', type=int, default=1) #是否使用证据网络
        parser.add_argument('--use_cql', type=int, default=1) #是否使用保守q学习
        parser.add_argument('--use_windowslen', type=int, default=1) #是否将上次推荐的一半加入滑动窗口
    ...
    def __init__(self, args, corpus):
        ...
        self.use_evidence = args.use_evidence
        self.use_cql = args.use_cql
        self.use_windowslen = args.use_windowslen
    ...
    def compute_evidence(self, action, item_embs):
        ...
        if not self.use_evidence:
            return torch.ones(item_embs.size(0), 5, device=item_embs.device)
        return self.evidence_network(action, item_embs)

    def calculate_vacuity(self, evidence):
        """计算vacuity"""
        if not self.use_evidence:
            return torch.zeros(evidence.size(0), device=evidence.device)
    ...
    def calculate_evidential_reward(self, ratings, vacuity, positive_threshold=3.0):
        ...
        if not self.use_evidence:
            return traditional_reward.mean()   # 去掉 λ*vacuity 项        
        # 不确定性奖励
        ...
        for i in range(B):
            ...
            self.sliding_window.update(user_id, item_emb.squeeze(), 
                                      recommended_items=[self.item_emb(torch.tensor([item]).long().to(self.device)) 
                                                         for item in recommended_items], use_windowslen=self.use_windowslen)
